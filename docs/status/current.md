...
- 独立 checker 源码快照身份 v0.1：闭合输入为除根 `.git` metadata 和 manifest 自身 sidecar 外的全部非目录 entry；路径限制为可移植 ASCII 组件并按 UTF-8 原始字节排序，只接受 `0644` / `0755` 普通文件，记录原始 byte length 与 SHA-256，并精确锁定 module、Go 语言基线和 toolchain 声明。canonical manifest 当前覆盖 652 个文件、177,483 bytes，`checker.source` 为 `sha256:6f52f4756b163868f54e6858f777be6894a7179e374ce9cbaf73592b247dcbf8`；Git commit / tree 只作来源追溯，不能替代该身份。
...
production conclusion 已经从 producer 自述中独立出来。下一步进入独立 check outcome、remaining trust / missing artifact 与四态 result 聚合；重点是接受 Evidence 对自身结论的忠实表达，而不是把 production `satisfied` 抄成 checker `accepted`。

1. 将已经实现的严格解析、身份、subject、义务重建、state / support、反例 / concrete replay、proof support 与 conclusion recompute 分别物化为确定的 check outcome / code / refs；已确认矛盾保持 `rejected`，缺少材料、能力或内部资源保持 `incomplete`，两者不得混用。
2. 从实际 proof finding、Evidence trust、request allowed trust 与 artifact 可得性独立形成 `remaining_trust` 和 `missing_artifacts`；当前 160 个 missing proof material 不能因为 producer `satisfied` 消失，65 个 attestation 也不能升级为独立 proof。
3. 按 ADR 0008 的唯一顺位聚合四态：任一确定拒绝优先，其次 incomplete / 缺失 / 不允许 trust，再次是允许且非空 remaining trust 的 `accepted-with-trust`，只有全部必需检查完成且 remaining trust 为空时才是 `accepted`。
4. 绑定 Evidence raw / document 双摘要、request、checker source / toolchain 与实际 TCB；独立 companion 不能写回 Evidence、复用 producer identity 或让 expected result 决定实际 outcome。
5. 覆盖正确报告 `violated` / `input_rejected` / `implementation_inconsistent` 仍可被独立接受、`certificate-required` + attestation 为 incomplete、attestation-allowed 保留 trust、缺失 artifact、内部资源不足、生产结论漂移和进程外层失败不伪造规范 result 的负例与确定性回归。

停止线：下一切片只形成内存中的 check / trust / missing / 四态聚合和 companion 所需的身份边界，不生成 canonical companion document，不实现产品 CLI，不构建或发布 checker binary / `checker.artifact`，不检查 minimality，不增加非空 kernel / certificate 能力，不执行 solver、生产 Node target 或生产编译器，也不产生六平台证据。

完成标准：每个进入结果层的锁定场景都由真实前置 finding 唯一决定 checks、remaining trust、missing 与四态 outcome；拒绝优先级、incomplete 与 accepted-with-trust 边界和 contract 负例一致，生产 conclusion 与独立 result 不混层；全量 test / vet、`checker.source` 与主仓库级门禁继续通过。

## 后续顺位

1. Go host/source 局部供应链门禁、checker request / bundle parser、`checker.source` v0.1、当前锁定 Axiom IR 结构 / 类型良构与有限执行、Axiom Evidence 结构 / 身份、obligation completeness、state / support、counterexample world / `WF`、concrete input / `Pre`、8 个 proof-failure target replay、host / golden output comparison、proof support 真值边界与 production conclusion 精确重算已经通过；下一步推进独立 check outcome、remaining trust / missing artifact 与四态 result 聚合，Git 对象 ID 与 payload acceptance 继续不能冒充协议源码身份。
2. 四态 result 聚合之后再收口 canonical companion 编码、完整累计资源与产品 CLI / artifact 边界；当前离线双仓工作不需要 checker remote，远程仓库创建、依赖安装、push、发布与部署仍分别提醒并授权。
...
- 表面语法；
- Axiom Evidence 的具体证明 certificate 格式、非空 kernel / certificate 能力，以及独立 checker 的四态 result、累计资源核算、`checker.artifact` 和 CLI 实现；
