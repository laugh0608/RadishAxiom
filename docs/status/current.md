...
- 独立 checker 源码快照身份 v0.1：闭合输入为除根 `.git` metadata 和 manifest 自身 sidecar 外的全部非目录 entry；路径限制为可移植 ASCII 组件并按 UTF-8 原始字节排序，只接受 `0644` / `0755` 普通文件，记录原始 byte length 与 SHA-256，并精确锁定 module、Go 语言基线和 toolchain 声明。canonical manifest 当前覆盖 658 个文件、178,622 bytes，`checker.source` 为 `sha256:5db26879872044fc68a42a0254eb43fe45b28b6dc61c063b82ca749cdbba6b1f`；Git commit / tree 只作来源追溯，不能替代该身份。
...
内存中的十类 check、remaining trust / missing artifact 与四态聚合已经形成。下一步收口 canonical companion document 与完整 invocation 边界；重点是让规范字节只编码已经形成的实际结果，并把 checker binary artifact、source、toolchain、TCB 与进程失败分层，不能用 source 或 expected result 补造运行事实。

1. 为已经验证的内存 `Result` 实现 Independent Check Contract v0.1 的闭合 canonical encoder 与严格 parser / self-check；按 check ID、missing digest、trust ID 和 TCB `(category, artifact)` 规范排序，重算 result document domain digest，不建立第二套聚合器。
2. 将 checker binary artifact 作为显式、不可缺省的运行输入绑定；它与 `checker.source`、Git commit / tree、Go toolchain 和 TCB component 身份不可互换。没有实际 binary digest 时只允许保留内存结果，不生成冒充正式运行的 companion。
3. 收口前置与调用失败：request / manifest / Evidence 非规范或摘要矛盾只能形成有真实 raw identity 的 rejected；内部资源不足只有在所需 document / checker 身份已经形成时才可为 incomplete；外层 kill、截断、无法读取 request 或无法确认 checker 身份只形成 invocation failure record，不伪造四态 result。
4. 将当前分散的 parser、artifact、semantic-step 与 logical-memory 消费累计到一次 invocation；内部 wall-clock 停止与操作系统外层时限继续分开，已有拒绝不得被后续资源不足覆盖。
5. 覆盖 25 个结果层场景的 canonical round-trip、严格 Evidence 拒绝 result、missing / disallowed trust / proof incomplete / conclusion drift、checker artifact / source / TCB 漂移、result ID / refs / 顺序与 100 次确定性负例；expected result 只作事后 oracle，不进入实际形成路径。

停止线：下一切片只实现 canonical companion / invocation / 累计资源边界，不实现产品 CLI，不构建或发布 checker binary / `checker.artifact`，不检查 minimality，不增加非空 kernel / certificate 能力，不执行 solver、生产 Node target 或生产编译器，也不产生六平台证据。

完成标准：具有完整实际身份的内存结果可唯一编码、严格重解析并重算 document digest；前置 rejection、内部 incomplete 与进程外 failure 不混层，source / binary / TCB 不混淆，结果字节不依赖 map、expected companion 或 producer conclusion；全量 test / vet、`checker.source` 与主仓库级门禁继续通过。

## 后续顺位

1. Go host/source 局部供应链门禁、checker request / bundle parser、`checker.source` v0.1、当前锁定 Axiom IR 结构 / 类型良构与有限执行、Axiom Evidence 结构 / 身份、obligation completeness、state / support、counterexample world / `WF`、concrete input / `Pre`、8 个 proof-failure target replay、host / golden output comparison、proof support 真值边界、production conclusion 精确重算，以及内存 check / trust / missing / 四态结果已经通过；下一步推进 canonical companion、invocation failure 与完整累计资源，Git 对象 ID 与 payload acceptance 继续不能冒充协议源码或 binary 身份。
2. canonical companion 与 invocation 边界之后再收口产品 CLI / artifact 构建；当前离线双仓工作不需要 checker remote，远程仓库创建、依赖安装、push、发布与部署仍分别提醒并授权。
...
- 表面语法；
- Axiom Evidence 的具体证明 certificate 格式、非空 kernel / certificate 能力，以及独立 checker 的 canonical companion result、累计资源核算、`checker.artifact` 和 CLI 实现；
