...
- 独立 checker 源码快照身份 v0.1：闭合输入为除根 `.git` metadata 和 manifest 自身 sidecar 外的全部非目录 entry；路径限制为可移植 ASCII 组件并按 UTF-8 原始字节排序，只接受 `0644` / `0755` 普通文件，记录原始 byte length 与 SHA-256，并精确锁定 module、Go 语言基线和 toolchain 声明。canonical manifest 当前覆盖 646 个文件、176,310 bytes，`checker.source` 为 `sha256:d8496c37c6dacd282749b583ee99d2ec992c240700dee3374a2ad3901f531448`；Git commit / tree 只作来源追溯，不能替代该身份。
...
具体输入、有限 IR、proof-failure target 与 host / golden output 动态关系已经形成干净提交。下一步进入 proof support 真值与可检查能力边界；重点是不让 `kernel-replay`、`backend-attestation`、prove execution 的 `completed` 或 cvc5 状态文本自证为独立 proof。

1. 盘点全部锁定 `proved` result 的 support kind、prove execution、obligation-set / query / response artifact、tool role、trust 与 artifact format / version，按稳定 ID 固定真实闭包和当前可独立消费的最小子集。
2. 通过 bundle verifier 受约束重开 query / response / support artifact，分别核对 raw identity、协议结构、logic / status、目标 obligation 与执行绑定；不读取 pipeline receipt 或 Evidence result 作为定理真值。
3. 对 `kernel-replay` 明确检查是否实际存在 checker 可消费的 proof / certificate 与完整规则覆盖。当前 certificate capability 集为空时，不得因 support tag 名称而假装已重演 kernel rule；缺失独立材料必须保持显式未验证边界。
4. 对 `backend-attestation` 只在精确 response、backend identity、execution 与 `proof-backend` trust 闭包内确认 attestation 事实，不能升级为无信任 proof；与 kernel / certificate 结果保持不同状态和说明。
5. 增加 query / response / obligation / support / trust 交叉绑定、错误 status、缺失 proof material、未知 capability、资源耗尽与早期拒绝负例，并保持 9 个 input failure、8 个 proof failure 与 3 个 output mismatch 的既有动态结果稳定。

停止线：本切片不修改公共 certificate 格式或空 capability 集，不把 backend attestation 升级为 proof，不重算 conclusion，不聚合独立四态 result，也不执行 solver、生产 Node target 或生产编译器。

完成标准：checker 能对每个锁定 proved support 给出与实际材料相称的独立检查结论：可验证的支持按显式 profile 重放，只有 attestation 的保持剩余 trust，缺少可检查 proof material 的明确失败关闭而非默认成功；全量 test / vet、`checker.source` 与主仓库级门禁继续通过，结论不超过实际 support 能力。

## 后续顺位

1. Go host/source 局部供应链门禁、checker request / bundle parser、`checker.source` v0.1、当前锁定 Axiom IR 结构 / 类型良构与有限执行、Axiom Evidence 结构 / 身份、obligation completeness、state / support、counterexample world / `WF`、concrete input / `Pre`、8 个 proof-failure target replay 与 host / golden output comparison 已经通过；下一步推进 proof support 真值边界，Git 对象 ID 与 payload acceptance 继续不能冒充协议源码身份。
2. proof support 真值边界之后再进入 conclusion recompute；当前离线双仓工作不需要 checker remote，远程仓库创建、依赖安装、push、发布与部署仍分别提醒并授权。
...
- 表面语法；
- Axiom Evidence 的具体证明 certificate 格式，以及独立 checker 的 proof / attestation 真值、conclusion / result、累计资源核算、`checker.artifact` 和 CLI 实现；
