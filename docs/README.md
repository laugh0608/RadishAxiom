...
- [ADR 0013：Darwin checker 强隔离宿主与虚拟执行边界](adr/0013-darwin-checker-hard-isolation.md)：拒绝 native best-effort / root broker fallback，选择 per-invocation signed App-Sandboxed Hypervisor runner 作为唯一实施候选，并冻结 guest memory、无设备、descriptor-fed identity 与迁移停止线。
