...
- [ADR 0010：独立 checker runtime payload 的持久发布与登记](adr/0010-checker-runtime-payload-durable-registration.md)：选择 GitHub immutable Release asset，冻结专用 tag / distribution package、登记状态机、独立回读与 append-only replacement；当前 source 的临时 distribution candidate 已形成并完成 provider 回读，但不表示仓库设置已经验证、Release 已创建或 runtime 已登记。
...
- 当前 `checker.source = sha256:401158...e3999` 的自包含 distribution candidate 已完成精确构建、payload / distribution acceptance、临时直传和按 artifact ID 独立回读；下一受控入口是另行验证并授权 immutable Release 设置与持久发布，之后才进入正式登记、launcher OS 限制和产品安装协调；
