...
- [ADR 0013：Darwin checker 强隔离宿主与虚拟执行边界](adr/0013-darwin-checker-hard-isolation.md)：拒绝 native best-effort / root broker fallback，选择 per-invocation signed App-Sandboxed Hypervisor runner 作为唯一实施候选，并冻结 guest memory、无 network / disk / host share、descriptor-fed identity 与迁移停止线。
- [ADR 0014：Darwin App Sandbox container 与 runner 无状态边界](adr/0014-darwin-app-sandbox-container-state.md)：接受固定产品 identity 的 OS-managed container skeleton 作为安装级 host state，同时禁止 runner 读写 container 或形成请求派生增量；inherit helper、XPC 与受权清理不作为 fallback。
...
- [Checker runtime 两层业务 manifest parser 切片审阅单](checker-runtime-manifest-parser-slice-review.md)：记录 distribution / candidate manifest 的 strict canonical JSON、archive / registration 双层身份、闭合 inventory、合成负例与 spawn 停止线。
- [Checker runtime immutable spawn plan 切片审阅单](checker-runtime-spawn-plan-slice-review.md)：记录 execution profile / 外层 limit 绑定、qualification / invocation 单一不可变配置、result-or-failure 排他状态机、合成进程负例与 Darwin 原生隔离停止线。
- [Checker runtime Darwin process isolation 原语审阅单](checker-runtime-darwin-process-isolation-review.md)：记录公开 spawn / stream / deadline / observation 能力的合成证据、App Sandbox 签名边界、hard memory / process tree / executable TOCTOU 阻断项与下一次授权停止线。
- [Checker runtime Darwin 强隔离架构审阅单](checker-runtime-darwin-hard-isolation-review.md)：比较 App Sandbox、privileged broker、Virtualization 与 Hypervisor 公开边界，记录 128 MiB / empty-network / read-only-share 和 suspended code identity 合成探针，并给出不启动 VM 的证据停止线。
...
- [Checker Runtime Payload Registration v0.1](../contracts/checker-runtime-payloads-v0.1/README.md)：闭合 checker source、target、artifact / provenance / acceptance、retention / fetch / 重新验证，以及 launcher / 安装 / qualification / 激活策略；当前 Rust 已实现 registry identity / target selection、严格内外层 USTAR 与业务 manifest、installation receipt、Darwin descriptor-backed 六项 store capability、单一 result consumer、immutable spawn plan 和外层排他状态机，原生 spawn / isolation 仍未实现；ADR 0013 已选择 virtualized hard-isolation 方向，synthetic Linux microguest feasibility 已通过，但 production runner / guest TCB、公共身份迁移与 qualification 尚未物化，active runtime 为 0。
...
- 当前阶段、精确实现范围、明日事项与停止线统一以[当前状态](status/current.md)为准，不在文档索引复制易漂移的源码摘要或提交身份；
- 当前 `checker.source = sha256:401158...e3999` 的自包含 distribution 已完成精确构建、payload / distribution acceptance、不可变发布、发布后独立回读与 `registered-inactive` 登记；launcher policy、产品实现宿主与持久化接口均已冻结，依赖无关的一致性核心已形成；Rust `1.97.1` 的 rustup component/source payload 验收、当前用户级五组件安装、strict policy / record identity、target selection、extraction-free USTAR、两层业务 manifest、installation receipt、单一 result consumer、immutable spawn plan / 外层排他状态机，以及 exact `libc 0.2.189` 私有 binding 支撑的 Darwin descriptor-backed 六项 store capability 均已完成；Darwin process 原语审阅确认公开 API 只能闭合 supervisor 子集，ADR 0013 已将 per-invocation signed App-Sandboxed Hypervisor runner 冻结为唯一实施候选，synthetic Linux microguest 已在单一当前主机形成 descriptor-fed、exact 128 MiB、真实进程 / 后代、syscall 拒绝、bounded stream 与 6 秒 cold teardown 动态证据，但真实 checker / bundle、production runner / guest TCB、生产签名、公共身份迁移和 qualification 仍未物化，真实 checker 安装与激活继续分别验证和授权；
