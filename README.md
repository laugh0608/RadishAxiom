...
项目处于设计阶段。生产 `raxc` 已选择 Rust，首个验证后端已选择独立进程运行的 cvc5 1.3.4，首个目标执行路径已选择 Node.js 24.19.0 LTS，首版编译管线已选择内容寻址、验证门控和可离线重放的显式制品 DAG；独立 checker 仍未冻结。在这些入口条件完成前，不进入编译器实现。
