...
其余 Go 五个平台、Rust standalone / 其他平台以及 cvc5、Node 的全部制品仍明确为 `not-downloaded`、`not-performed` 和 `not-accepted`；不因工具版本相同、source 或相邻平台已经验收而外推。Rust rustup distribution 单独绑定 `channel-rust-1.97.1.toml` 的原始摘要、`2026-07-16` channel date、manifest version `2`、`minimal` profile、`aarch64-apple-darwin` target 和两个显式 quality component。registry 中的 `installation = not-authorized` 是该 payload acceptance / registry 批次不自行授予安装权限的闭合边界，不是开发主机的可变 rustup inventory；当前用户级安装观察以[首个 Rust 纵向切片审阅单](../../docs/checker-runtime-rust-first-slice-review.md)和[当前状态](../../docs/status/current.md)为准。Rust 工具级依赖与许可证审阅只标记为 `partial-accepted-set-only`，source 中观察到的多种 SPDX 表达式不是整包许可证结论。
...
首个 Rust 纵向切片的工具链、候选依赖、零第三方依赖决策、crate 边界与后续授权停止线见 [Checker runtime 首个 Rust 纵向切片审阅单](../../docs/checker-runtime-rust-first-slice-review.md)。standalone archive 与 rustup component 继续是不可互换的字节链；component acceptance 与当前用户级精确工具链安装已经分别完成，下一受控动作是 workspace / toolchain file / Cargo 生成 lockfile 与首个零第三方依赖实现切片。
