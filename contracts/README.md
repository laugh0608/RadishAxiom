...
- [Checker Runtime Payload Registration v0.1](checker-runtime-payloads-v0.1/README.md)：绑定 checker source、target、artifact / provenance / acceptance、候选 / durable provider、字节 retention / fetch 与登记状态机；最近可取得的有限期候选仍绑定旧 source，当前 source 的 distribution 实现已经进入默认分支但尚未形成候选，GitHub immutable Release provider 已选但设置尚未验证且 Release 未物化，active runtime 为 0。
