...
- [Toolchain Payload Acceptance v0.1](toolchain-payload-acceptance-v0.1/README.md)：逐 payload 绑定 publisher / 项目摘要、只读 archive 观察、依赖 / 许可证库存、签名状态与受限最终 acceptance；当前接受 Go `go1.26.7` macOS arm64 host/source 与 Rust `1.97.1` rustup component/source。
- [Checker Runtime Payload Registration v0.1](checker-runtime-payloads-v0.1/README.md)：绑定 checker source、target、artifact / provenance / acceptance、候选 / durable provider、字节 retention / fetch、登记状态机与 launcher policy；当前精确 distribution 已不可变发布并登记为 `registered-inactive`，Rust 已有 Unix 临时根 store 最小事务，但生产 installation / launcher / qualification 尚未实现，active runtime 为 0。
