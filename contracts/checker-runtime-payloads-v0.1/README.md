...
主仓 Rust 产品组件现已实现同一确定性 USTAR header profile 与 extraction-free 闭合清单原语：严格核对内层四成员和外层六成员的路径、普通文件类型、uid / gid / mtime / user / group / device 字段、checksum、mode、长度、SHA-256、顺序、padding 与精确双零 trailer，并直接借用成员字节供下一层复核。两层业务 manifest consumer 随后严格解析 retention / distribution `0.1` canonical JSON，闭合父归档、ordered content、checker / target 与 registration 中 distribution、candidate、manifest、acceptance、provenance 和 executable 身份；USTAR 成功不能替代该业务验证。installation receipt 以当前登记记录闭合 artifact、checker、distribution、provider、registration、target、slot、verifier、安装时间与 `installed-inactive`，并与 Python oracle 的 1,797-byte 黄金结果逐字一致。首个 store 最小事务曾以调用方注入的 Unix 临时根闭合真实 OS target lock、owned staging、同文件系统 publish、exact reuse / mismatch、slot read 与 held-lock recovery；两种实现对合成 slot 得到相同 tree digest。后续 Darwin store 切片已用 descriptor-relative、no-replace 与 full-sync 原语替代 macOS core 的较弱 path-based 路径，非 Darwin 路径仍只是平台候选。当前实现不读取真实产品根或真实 Release asset，也不解析 provenance / acceptance 正文；完整 policy 状态和 active count 因此不变。

后续 Rust 切片已补齐单一 Independent Check result consumer：对公共 result `0.1` 严格复算 check ID、四态聚合与 result document digest，绑定 registration / request / Evidence / 当前单体四类 TCB，并以不同类型解析 request-bound `axiom-checker-invocation-failure` `0.1`。qualification 输入现在先经过该 consumer，再进入 scenario / raw digest / outcome 的 policy 绑定。测试独立接受全部 28 份公共 canonical result 并拒绝 7 份 result 负例；这仍不重放 checker 语义规则。
