...
当前外层 distribution 已包含内层候选、distribution acceptance / manifest、checker Apache-2.0 `LICENSE` 与 Go `LICENSE` / `PATENTS` 六个成员，并由独立 acceptance 接受为受控 durable publication candidate；这只完成状态机的 `distribution-package-accepted`。之后仍须经单独授权先验证并启用 immutable releases，再走 draft → 一次发布 → provider verification → 独立回读；当前没有创建或授权 Release、tag 或 durable asset upload。
